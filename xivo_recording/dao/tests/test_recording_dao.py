...
class TestRecordingDao(unittest.TestCase):
    ...
    def test_recording_db(self):
        ...
        self.assert_(self.contains(records, lambda record: record.to_string() == expected_object.to_string()), "Write/read from database failed")

    def contains(self, listing, filter_fct):
        for x in listing:
            if filter_fct(x):
                return True
        return False
