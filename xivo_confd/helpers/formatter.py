...
class Formatter(object):
    ...
    def to_model(self, api_data):
        ...
        return self.dict_to_model(data_dict)
    ...
    def update_model(self, api_data, model_to_update):
        data_dict = self._serializer.decode(api_data)
        return self.update_dict_model(data_dict, model_to_update)
...
class NewFormatter(object):

    def __init__(self, view_document, serializer, model_class, links_func):
        self._view_document = view_document
        self._serializer = serializer
        self._model_class = model_class
        self._links_func = links_func
    ...
    def dict_to_model(self, data_dict):
        mapped_dict = mapper.map_to_model(self._view_document.mapping, data_dict)
        model_class = self._model_class.from_user_data(mapped_dict)
        return model_class
    ...
    def _prepare_model_to_api(self, model):
        model_dict = model.to_user_data()
        mapped_dict = mapper.map_to_api(self._view_document.mapping, model_dict)
        mapped_dict.update(self._links_func(model))
        return mapped_dict
