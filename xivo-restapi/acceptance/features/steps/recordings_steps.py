...
def when_i_save_call_details_for_a_call_referenced_by_its_group1_in_campaign_group2_replied_by_agent_with_number_group3(step, callid, campaign_name, agent_no):
...
@step(u'Given there is a recording referenced by a "([^"]*)" with agent "([^"]*)" in campaign "([^"]*)"')
def given_there_is_a_recording_referenced_by_a_callid(step, local_callid, agent_no, campaign_name):
    campaign_id = record_campaigns_dao.id_from_name(campaign_name)
