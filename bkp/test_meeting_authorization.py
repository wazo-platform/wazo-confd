...
def until_event(accumulator, headers_matchers, message_matcher, timeout=5):
    def assert_function():
        assert_that(
            ...
            has_items(
                has_entries(
                    headers=has_entries(headers_matchers or {}),
                    message=has_entries(message_matcher or {}),
                ),
            ),
        )

    until.assert_(assert_function, timeout=timeout)
