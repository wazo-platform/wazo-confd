...
class TestProvCodeAvailble(unittest.TestCase):
    ...
    def test_given_no_lines_with_code_then_validation_passes(self):
        ...
        line = Mock(Line, provisioning_code="123456")

        self.validator.validate(line)

        self.dao.find_by.assert_called_once_with("provisioning_code", "123456")
...
class TestProvCodeChanged(unittest.TestCase):
    ...
    def test_given_code_has_not_changed_then_validation_passes(self):
        line = Mock(Line, id=1, provisioning_code="123456")
    ...
    def test_given_code_has_changed_and_is_available_then_validation_passes(self):
        existing_line = Mock(Line, id=1, provisioning_code="123456")
        new_line = Mock(Line, id=1, provisioning_code="234567")
        ...
        self.dao.find_by.assert_called_once_with("provisioning_code", "234567")

    def test_given_code_has_changed_and_same_code_exists_then_validation_fails(self):
        old_line = Mock(Line, id=1, provisioning_code="123456")
        new_line = Mock(Line, id=1, provisioning_code="234567")
        existing_line = Mock(Line, id=2, provisioning_code="234567")
