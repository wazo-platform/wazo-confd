...
def test_post_errors():
    ...
    s.check_bogus_field_returns_error(url, 'name', s.random_string(21))
...
def unique_error_checks(url):
    with fixtures.moh(name='unique') as moh:
        s.check_bogus_field_returns_error(url, 'name', moh['name'], {'mode': 'files'})



def test_search():
    with fixtures.moh(name='visible', label='hello') as visible, fixtures.moh(name='hidden', label='hidden') as hidden:
        url = confd.moh
        searches = {
            'name': 'visible',
            'label': 'hello',
...
def test_sorting_offset_limit():
    with fixtures.moh(name='sort1') as moh1, fixtures.moh(name='sort2') as moh2:
        url = confd.moh.get
        s.check_sorting(url, moh1, moh2, 'name', 'sort', 'uuid')

        s.check_offset(url, moh1, moh2, 'name', 'sort', 'uuid')
        s.check_offset_legacy(url, moh1, moh2, 'name', 'sort', 'uuid')

        s.check_limit(url, moh1, moh2, 'name', 'sort', 'uuid')



def test_list_multi_tenant():
    with fixtures.moh(wazo_tenant=MAIN_TENANT) as main, fixtures.moh(wazo_tenant=SUB_TENANT) as sub:
...
def test_edit_name_unavailable():
    with fixtures.moh(name='OriginalName') as moh:
...
def test_edit_custom_mode_with_application():
    with fixtures.moh(mode='files', application=None) as moh:
        response = confd.moh(moh['uuid']).put(mode='custom', application='/bin/false')
        response.assert_updated()



def test_edit_custom_mode_without_application():
    with fixtures.moh(mode='files', application=None) as moh:
...
def test_add_update_delete_filename():
    with fixtures.moh() as moh:
...
def test_add_update_delete_filename_multi_tenant():
    with fixtures.moh(tenant_uuid=MAIN_TENANT) as moh:
        client = _new_moh_file_client()
...
def test_bus_events():
    with fixtures.moh() as moh:
        s.check_bus_event('config.moh.created', confd.moh.post, {'name': 'bus_event', 'mode': 'files'})
        s.check_bus_event('config.moh.edited', confd.moh(moh['uuid']).put)
        s.check_bus_event('config.moh.deleted', confd.moh(moh['uuid']).delete)
