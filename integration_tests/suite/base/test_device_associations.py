...
def test_when_caller_id_updated_on_line_then_provd_is_updated(
    ...
    with a.user_line(user, line), a.line_extension(line, extension), a.line_device(
        line, device
    ):
        ...
        assert_that(
            sip_line,
            has_entries({'display_name': 'jôhn smîth', 'number': extension['exten']}),
        )
...
def test_when_caller_id_updated_on_user_then_provd_is_updated(
    ...
    with a.user_line(user, line), a.line_extension(line, extension), a.line_device(
        ...
        assert_that(
            sip_line,
            has_entries({'display_name': 'rôger rabbit', 'number': extension['exten']}),
        )
...
def test_dissociate_sccp_endpoint_associated_to_device(
    user, line, extension, sccp, device
):
