...
class UniqueGlobalPerContext(Validator):
    ...
    def validate(self, model):
        voicemail = self.dao.find_by(global_=True, context=model.context)
        if voicemail and model.global_ and voicemail.id != model.id:
            raise errors.not_permitted(
                'There can be only one global voicemail per tenant.'
            )
...
class UserVoicemailIsGlobal(Validator):
    def validate(self, voicemail):
        if voicemail.global_ and len(voicemail.users) > 0:
            raise errors.not_permitted(
