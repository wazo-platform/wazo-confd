...
def test_get_errors():
    fake_blocklist_number = confd.blocklist.numbers(FAKE_UUID).get
...
def test_list(user):
    ...
    with numbers[0] as first, numbers[1] as second:
        response = confd.blocklist.numbers.get()
        assert_that(
            response.items,
            has_items(
                has_entries(
...
def test_sorting_offset_limit(user):
    ...
    with numbers[0] as blocklist_number1, numbers[1] as blocklist_number2:
        url = confd.blocklist.numbers.get
...
def test_get(user):
    ...
    with fixtures.users_blocklist_number(
        ...
        response = user_confd_client.blocklist.numbers(blocklist_number['uuid']).get()
        assert_that(
            response.item,
            has_entries(
