...
def test_associate_errors():
    with fixtures.trunk() as trunk, fixtures.register_sip() as register:
...
def test_associate_multiple_register_sip_to_trunk():
    with fixtures.trunk() as trunk, fixtures.register_sip() as register1, fixtures.register_sip() as register2:
...
def test_associate_multiple_trunks_to_register_sip():
    with fixtures.trunk() as trunk1, fixtures.trunk() as trunk2, fixtures.register_sip() as register:
...
def test_associate_when_endpoint_custom():
    with fixtures.trunk() as trunk, fixtures.register_sip() as register, fixtures.custom() as custom:
        with a.trunk_endpoint_custom(trunk, custom):
            response = confd.trunks(trunk['id']).registers.sip(register['id']).put()
            response.assert_match(400, e.resource_associated('Trunk', 'Endpoint'))


def test_associate_when_endpoint_iax():
    with fixtures.trunk() as trunk, fixtures.register_sip() as register, fixtures.iax() as iax:
...
def test_get_register_sip_relation():
    with fixtures.trunk() as trunk, fixtures.register_sip() as register:
        with a.trunk_register_sip(trunk, register):
