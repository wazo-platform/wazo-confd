...
def test_get_destinations():
    ...
    assert_that(
        ...
        contains_inanyorder(
            ...
            has_entries(
                {'type': 'bsfilter', 'parameters': [{'name': 'filter_member_id'}]}
            ),
            has_entries(
                {'type': 'conference', 'parameters': [{'name': 'conference_id'}]}
            ),
            has_entries({'type': 'custom', 'parameters': [{'name': 'exten'}]}),
            has_entries(
                {
                    'type': 'forward',
                    'parameters': contains_inanyorder(
                        {
                            'values': ['busy', 'noanswer', 'unconditional'],
                            'name': 'forward',
                        },
                        {'name': 'exten'},
                    ),
            ...
            has_entries({'type': 'group', 'parameters': [{'name': 'group_id'}]}),
            has_entries(
                {
                    'type': 'groupmember',
                    'parameters': contains_inanyorder(
            ...
            has_entries(
                {
                    'type': 'transfer',
                    'parameters': [
                        {'values': ['blind', 'attended'], 'name': 'transfer'}
                    ],
                }
            ),
            has_entries(
