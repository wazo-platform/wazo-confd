...
class TestCallLogs(unittest.TestCase):
    ...
    def setUp(self):
        self.client = BaseIntegrationTest.new_client(
            headers={'Accept': 'text/csv', 'X-Auth-Token': 'valid-token'}
        ).url

    def test_missing_start_date(self):
        response = self.client.call_logs.get(end_date='2013-01-29T00:00:00')
        response.assert_match(400, e.missing_parameters('start_date'))
    ...
    def test_empty_start_date(self):
        response = self.client.call_logs.get(start_date='', end_date='2013-01-29T00:00:00')
        response.assert_match(400, e.missing_parameters(field='start_date', type='datetime'))

    def test_empty_end_date(self):
        response = self.client.call_logs.get(start_date='2013-01-29T00:00:00', end_date='')
    ...
    def test_list_end_sooner_than_start(self, _):
        response = self.client.call_logs.get(
            start_date='2013-01-31T00:00:00',
            end_date='2013-01-30T00:00:00',
        )

        assert_that(response.csv(), empty())
    ...
    @fixtures.call_log(
        date=datetime(2013, 1, 30, 11, 13, 47),
        date_answer=None,
    ...
    @fixtures.call_log(
        date=datetime(2013, 1, 31, 11, 20, 8),
    ...
    def test_when_list_in_period_then_call_logs_are_filtered(self, *_):
        response = self.client.call_logs.get(
            start_date='2013-01-30T11:11:11',
            end_date='2013-01-30T12:12:12',
        )
        assert_that(
            response.csv(),
            contains(
                {
                    'Call Date': '2013-01-30T11:13:47',
                    'Caller': 'Bob Marley (1002)',
                    'Called': '4185550155',
                    'Period': '0',
                    'user Field': '',
