...
def test_associate_multiple_outcalls_to_call_permission(
    outcall1, outcall2, outcall3, call_permission
):
    confd.outcalls(outcall1['id']).callpermissions(
        call_permission['id']
    ).put().assert_updated()
    confd.outcalls(outcall2['id']).callpermissions(
        call_permission['id']
    ).put().assert_updated()
    confd.outcalls(outcall3['id']).callpermissions(
        call_permission['id']
    ).put().assert_updated()
...
def test_dissociate(outcall, call_permission):
    with a.outcall_call_permission(outcall, call_permission, check=False):
        response = (
            confd.outcalls(outcall['id'])
            .callpermissions(call_permission['id'])
            .delete()
        )
...
def test_get_outcalls_relation(outcall, call_permission):
    with a.outcall_call_permission(outcall, call_permission):
        ...
        assert_that(
            response.item['outcalls'],
            contains_exactly(has_entries(id=outcall['id'], name=outcall['name'])),
        )
