...
class CreateAccessFeatureUniqueHostValidator(Validator):
    ...
    def validate(self, access_feature):
        existing = self.dao.find_by(host=access_feature.host,
                                    feature=access_feature.feature)

        if existing:
            raise errors.resource_exists('AccessFeatures',
...
def build_validator():
    return ValidationGroup(
        create=[
            CreateAccessFeatureUniqueHostValidator(access_feature_dao),
        ],
        edit=[
            EditAccessFeatureUniqueHostValidator(access_feature_dao),
        ]
    )
