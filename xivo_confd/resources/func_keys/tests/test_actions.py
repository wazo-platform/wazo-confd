...
from xivo_confd.resources.func_keys import actions
...
class TestFuncKeyActions(unittest.TestCase):
    ...
    @patch('xivo_confd.resources.func_keys.actions.make_response')
    @patch('xivo_confd.resources.func_keys.actions.converter.encode_list')
    @patch('xivo_dao.data_handler.func_key.services.search')
    @patch('xivo_confd.resources.func_keys.actions.request')
    def test_list(self, request, func_key_search, encode_list, make_response):
        request.args = {'limit': '1'}
        search_result = func_key_search.return_value = Mock(SearchResult)
        formatted_list = encode_list.return_value = Mock()
        response = make_response.return_value = Mock()

        result = actions.list()

        func_key_search.assert_called_once_with(limit=1)
        encode_list.assert_called_once_with(search_result.items, search_result.total)
        make_response.assert_called_once_with(formatted_list, 200)
        assert_that(result, equal_to(response))

    @patch('xivo_confd.resources.func_keys.actions.make_response')
    @patch('xivo_confd.resources.func_keys.actions.converter.encode')
    @patch('xivo_dao.data_handler.func_key.services.get')
    def test_get(self, func_key_get, converter_encode, make_response):
        func_key_id = 1

        func_key = func_key_get.return_value = Mock(FuncKey)
        formatted_func_key = converter_encode.return_value = Mock()
        response = make_response.return_value = Mock()

        result = actions.get(func_key_id)

        func_key_get.assert_called_once_with(func_key_id)
        converter_encode.assert_called_once_with(func_key)
