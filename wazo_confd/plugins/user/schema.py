...
class UserSchemaV2(BaseSchema):
    id = mallow.fields.Integer(dump_only=True)
    uuid = mallow.fields.String(dump_only=True)
    firstname = mallow.fields.String(validate=Length(max=128), required=True)
    lastname = mallow.fields.String(validate=Length(max=128), allow_none=True)
    email = mallow.fields.Email(validate=Length(max=254), allow_none=True)
    ...
    call_record_enabled = StrictBooleanV2()
    call_record_outgoing_external_enabled = StrictBooleanV2()
    call_record_outgoing_internal_enabled = StrictBooleanV2()
    call_record_incoming_internal_enabled = StrictBooleanV2()
    call_record_incoming_external_enabled = StrictBooleanV2()
    online_call_record_enabled = StrictBooleanV2()
    supervision_enabled = StrictBooleanV2()
    ring_seconds = mallow.fields.Integer(validate=Range(min=0, max=10800))
    simultaneous_calls = mallow.fields.Integer(validate=Range(min=1, max=20))
    call_permission_password = mallow.fields.String(
        validate=Regexp(CALL_PERMISSION_PASSWORD_REGEX), allow_none=True
    )
    subscription_type = mallow.fields.Integer(validate=Range(min=0, max=10))
    created_at = mallow.fields.DateTime(dump_only=True)
    enabled = StrictBooleanV2()
...
class UserSchemaNullableV2(UserSchemaV2):
